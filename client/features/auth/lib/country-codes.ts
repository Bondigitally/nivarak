...
export const COUNTRY_DIAL_CODES: CountryDialCode[] = [
  ...
  { iso: 'AS', name: 'American Samoa', dialCode: '+1684' },
  { iso: 'AD', name: 'Andorra', dialCode: '+376' },
  { iso: 'AO', name: 'Angola', dialCode: '+244' },
  { iso: 'AI', name: 'Anguilla', dialCode: '+1264' },
  ...
  { iso: 'BM', name: 'Bermuda', dialCode: '+1441' },
  ...
  { iso: 'VG', name: 'British Virgin Islands', dialCode: '+1284' },
  ...
  { iso: 'KY', name: 'Cayman Islands', dialCode: '+1345' },
  ...
  { iso: 'MK', name: 'Macedonia', dialCode: '+389' },
  ...
  { iso: 'MP', name: 'Northern Mariana Islands', dialCode: '+1670' },
  ...
  { iso: 'SX', name: 'Sint Maarten', dialCode: '+1721' },
  ...
  { iso: 'SZ', name: 'Swaziland', dialCode: '+268' },
  ...
  { iso: 'TC', name: 'Turks and Caicos Islands', dialCode: '+1649' },
  { iso: 'TV', name: 'Tuvalu', dialCode: '+688' },
  { iso: 'VI', name: 'U.S. Virgin Islands', dialCode: '+1340' },
  ...
  { iso: 'VA', name: 'Vatican', dialCode: '+39' },
...
export function getDefaultCountry(): CountryDialCode {
  return getCountryByIso(DEFAULT_COUNTRY_ISO) ?? COUNTRY_DIAL_CODES[0]!;
}

/** India first (product default), then remaining countries A–Z */
export const COUNTRY_DIAL_CODES_ORDERED: CountryDialCode[] = [
  ...(getCountryByIso(DEFAULT_COUNTRY_ISO)
    ? [getCountryByIso(DEFAULT_COUNTRY_ISO)!]
    : []),
  ...COUNTRY_DIAL_CODES.filter((country) => country.iso !== DEFAULT_COUNTRY_ISO),
];

/**
 * Typical mobile national-number length (digits after dial code).
 * Used to cap input and validate. Falls back to E.164 (15 − country-code digits).
 */
const NATIONAL_LENGTH_BY_ISO: Record<string, { min: number; max: number }> = {
  // South Asia
  IN: { min: 10, max: 10 },
  PK: { min: 10, max: 10 },
  BD: { min: 10, max: 10 },
...
function e164NationalMax(dialCode: string): number {
  const codeDigits = dialCode.replace(/\D/g, '').length;
  return Math.max(4, 15 - codeDigits);
}

/** National digit length range for a country (after dial code). */
export function getNationalLength(country: CountryDialCode): {
  min: number;
  max: number;
} {
  const known = NATIONAL_LENGTH_BY_ISO[country.iso];
  if (known) return known;

  // NANP territories stored as +1NXX — local subscriber is 7 digits
  if (/^\+1\d{3}$/.test(country.dialCode)) {
    return { min: 7, max: 7 };
  }

  const max = e164NationalMax(country.dialCode);
  return { min: Math.min(6, max), max };
}
